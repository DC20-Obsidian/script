...
class Args:
    def __init__(self, default_page: int):
        ...
        parser.add_argument(
            "-a", "--all", action="store_true", help="use all page (configured by type)"
        )
        ...
        parser.add_argument(
            ...
            "--unprocessed",
            action="store_true",
            help="only output unprocessed text fragments (implies --raw)",
        )
        parser.add_argument(
            "-f", "--file", help="filtered JSON containing PDF data to use"
        )
        parser.add_argument(
            "-s",
            "--saved",
